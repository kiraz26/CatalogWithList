...
public class Catalog {
	...
	public String getItemDetails(String item) {
		...
		if (items.contains(item)) {
			...
			result += items.get(items.indexOf(item)) + "-" + prices.get(items.indexOf(item)) + "-"
					+ monthlyPayments.get(items.indexOf(item));
			return result;
		}
	...
	public ArrayList<String> getItemsLessThanAMonthlyPrice(double price) {
		...
		for (int i = 0; i < monthlyPayments.size(); i++) {
			if (monthlyPayments.get(i) < price) {
				a.add(items.get(i) + "-" + prices.get(i) + "-" + monthlyPayments.get(i));
	...
	public void updatePrice(String item, double newPrice) {
		...
		if (items.contains(item)) {
			...
			prices.remove(a);
			prices.add(a, newPrice);
			monthlyPayments.remove(a);
			monthlyPayments.add(a, newPrice/12);
		}
